    ...
    public static int maxOfPoint(List<int[]> setOfPoint){
        ...
        for(int i = 0 ; i < setOfPoint.size() ; i++){
            for(int j = i+1 ; j < setOfPoint.size() ; j++){
                double slope = findSlope(setOfPoint.get(i)[0],setOfPoint.get(i)[1],setOfPoint.get(j)[0],setOfPoint.get(j)[1]);
                if(map.containsKey(slope)){
                    ...
                    int finalI = i;
                    int finalJ = j;
                    map.put(slope,new ArrayList<>(){{add(setOfPoint.get(finalI));add(setOfPoint.get(finalJ));}});
                }
            }
        }
